...
<template>
  <div class="flex flex-col gap-4 text-xs">
    ...
    <div class="flex flex-col gap-1.5">
      ...
      >
        <option value="hackrf">📡 HackRF One (C++ 原生硬件源)</option>
        <option value="file">📁 File Source (本地 IQ / WAV 录音文件)</option>
        <option value="rtlsdr">📡 RTL-SDR (C++ 原生硬件源)</option>
        <option value="bladerf">📡 BladeRF (C++ 原生硬件源)</option>
        <option value="limesdr">📡 LimeSDR (C++ 原生硬件源)</option>
        <option value="plutosdr">📡 ADALM-PLUTO (IIO 硬件源)</option>
    ...
    <div v-if="sourceConfig.type === 'hackrf' || sourceConfig.type === 'rtlsdr'" class="flex flex-col gap-1.5 p-3 bg-sdr-dark/60 rounded-md border border-sdr-border">
      <div class="flex items-center justify-between">
        ...
        >
          ...
          <span>{{ isScanningDevices ? (currentLang === 'zh' ? '正在扫描...' : 'Scanning...') : (currentLang === 'zh' ? '刷新设备' : 'Refresh') }}</span>
        </button>
      </div>
      ...
      <div class="flex flex-col gap-1 pt-1">
        ...
        >
          <option v-if="availableDevices.length === 0" value="">
            {{ isBackendConnected ? '未检测到设备 (请插入 USB 并点击刷新)' : 'C++ 后端未运行 (默认自动识别)' }}
          </option>
